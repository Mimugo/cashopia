...
import { categorizeTransaction, getCategorizationPatterns } from '@/lib/categories';
...
function detectDateColumn(headers: string[], samples: any[]): string | null {
  const dateKeywords = ['date', 'time', 'posted', 'transaction date', 'datetime'];
...
function detectDescriptionColumn(headers: string[]): string | null {
  const descKeywords = ['description', 'memo', 'details', 'merchant', 'payee', 'name'];
...
function detectBalanceColumn(headers: string[]): string | null {
  const balanceKeywords = ['balance', 'running balance', 'account balance', 'current balance'];
  ...
  return new Promise((resolve) => {
    ...
    Papa.parse(csvContent, {
      ...
      complete: async (results) => {
        ...
        for (const row of results.data as CsvRow[]) {
          try {
            ...
            if (mapping.typeColumn && row[mapping.typeColumn]) {
              ...
              if (typeValue.includes('credit') || typeValue.includes('income') || amount > 0) {
                type = 'income';
              }
            } else if (amount > 0) {
              type = 'income';
        ...
        resolve({ success: true, imported, failed, batchId: importBatchId, finalBalance: lastBalance });
      },
      error: (error) => {
